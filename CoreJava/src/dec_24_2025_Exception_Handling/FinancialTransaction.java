...
-> Implement a processTransaction method that:
   ...
   Accepts user input for transaction details such as transaction amount and account 	number. Validate the transaction amount and account number.
...
public class FinancialTransaction {
	public void processTransaction(double amount, long acNumber)  {
		if(amount<0) {
			throw new IllegalArgumentException("Error processing transaction: \nTransaction amount must be positive.");
		}
		else {
			IO.println("Processing transaction...");
			IO.println("Transaction successful: Amount Rs. "+amount +"transferred to account "+acNumber);
		}
	}
}
